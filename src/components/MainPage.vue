...
// Define a reactive variable for albums
const albums = ref<Album[]>([]);

// Function to fetch albums from the API
const fetchAlbums = async () => {
  try {
    const response = await axios.get<Album[]>('http://localhost:5091/TodoItems');
    albums.value = response.data; // Update the reactive variable with fetched data
  } catch (error) {
          ...
          <div class="w-full flex justify-center">
            <div class="max-w-screen w-full">
              <div class="grid grid-cols-7 gap-2">
                <div v-for="(album, index) in albums" :key="index" class="flex items-center justify-center mb-2 mt-2">
                  <div class="flex flex-col items-center">
                    ...
                    <div class="text-white text-sm font-arial">{{ album.name }}</div>
                  </div>
                </div>
              </div>
            </div>
